...
function AthleteController({ skills }: AthleteControllerProps) {
  ...
  useFrame((state) => {
    ...
    const interpolatePosition = (time: number): AthletePosition => {
      ...
      for (let i = 0; i < timestamps.length - 1; i++) {
        if (time >= timestamps[i] * JumpPhase && time <= timestamps[i + 1] * JumpPhase) {
          idx = i;
          break;
        }
      }
      ...
      return {
        ...
        joints: {
          leftShoulder: interpolate(currentPosition.joints.leftShoulder, nextPosition.joints.leftShoulder, factor),
          rightShoulder: interpolate(currentPosition.joints.rightShoulder, nextPosition.joints.rightShoulder, factor),
          leftThigh: interpolate(currentPosition.joints.leftThigh, nextPosition.joints.leftThigh, factor),
          rightThigh: interpolate(currentPosition.joints.rightThigh, nextPosition.joints.rightThigh, factor),
          leftShin: interpolate(currentPosition.joints.leftShin, nextPosition.joints.leftShin, factor),
          rightShin: interpolate(currentPosition.joints.rightShin, nextPosition.joints.rightShin, factor),
        }
      };
    };
    ...
    if (originalTime <= JumpPhase) {
      ...
      athletePositionRef.current.twist = lastPos.twist + cumulativeTwistRef.current;
      athletePositionRef.current.joints = {
        leftShoulder: interpolate(lastPos.joints.leftShoulder, firstPos.joints.leftShoulder, bounceProgress),
        rightShoulder: interpolate(lastPos.joints.rightShoulder, firstPos.joints.rightShoulder, bounceProgress),
        leftThigh: interpolate(lastPos.joints.leftThigh, firstPos.joints.leftThigh, bounceProgress),
        rightThigh: interpolate(lastPos.joints.rightThigh, firstPos.joints.rightThigh, bounceProgress),
        leftShin: interpolate(lastPos.joints.leftShin, firstPos.joints.leftShin, bounceProgress),
        rightShin: interpolate(lastPos.joints.rightShin, firstPos.joints.rightShin, bounceProgress),
      };
    }
  });
